...
t_instruccion_worker parsear_instruccion(char* linea, char** arg1, char** arg2, char** arg3) {
...
void activar_QI(int conexion_master, int conexion_storage){
    ...
    while (worker_activo){
        ...
        char* operacion_recibida = strdup("TRUNCATE mi_archivo:mi_tag 1024");

        if (operacion_recibida != NULL){
            char* arg1;
            char* arg2;
            char* arg3;
            t_instruccion_worker instruccion = parsear_instruccion(operacion_recibida, &arg1, &arg2, &arg3);
